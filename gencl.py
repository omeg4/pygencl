...
gendir = 'autogen/'
cldict = { 'ds' : 'bash-datsci.tex', 'fin' : 'bash-finance.tex' }
...
def gencl(cltdkey, compname, jobtitle):
    ...
    # Generate a unique file name, something like {date}_{CompName}_{JobTitl}
    ...
    try:
        sp.run(['cp', fname + '.pdf', '/home/matt/datadrop/Dropbox/jobapps/covletts/' + fname + '.pdf'], cwd=gendir, check=True)
    except CalledProcessError:
        return 'something went wrong when moving the PDF to dropbox!'
    return True

if __name__ == '__main__':
    cname = input('Enter company name: ')
    title = input('Enter job title: ')
    cldictkey = input('Cover letter dict key?\nds for data science template\nfin for finance template\n>> ')
